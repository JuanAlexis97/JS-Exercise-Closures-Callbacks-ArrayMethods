...
function getFullNames(runners) {
  ...
  runners.forEach(names => { 
    return runnerNames.push(`${names.last_name},${names.first_names}`)
  })
  return runnerNames;
}
...
function firstNamesAllCaps(runners) {
  return runners.map(function(firstNames){
    return firstNames.first_names.toUppercase();
  });
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let runnerSize = runners.filter(function(sizes){
    return sizes.shirt_sizes === tShirtSize;
  })
  return runnerSize;
}
